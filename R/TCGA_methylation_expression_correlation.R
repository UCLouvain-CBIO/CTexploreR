...
#'     default)
#'     
#' @param min_probe_number_to_include `numeric(1)` indicating the minimum 
...
#'     for the gene in selected tumors are returned in a tibble
#'     instead. 
#'
#' @export
#'
...
#'  min_probe_number_to_include = 1, return = FALSE)
TCGA_methylation_expression_correlation <- function(
    tumor = "all",
    gene = NULL,
    nt_up = 1000,
    nt_down = 200,
    min_probe_number_to_include = 3,
    ...
    methylation_expression <- methylation_expression[
      methylation_expression$probe_number >= min_probe_number_to_include, ]
    methylation_expression <- methylation_expression[
      !is.na(methylation_expression$met), ]
    
    if (nrow(methylation_expression) == 0) stop("less than ",  
                                                min_probe_number_to_include, 
