...
#' @description
#'
#' Plots a heatmap of genes expression in fetal germ cells,
...
#'
#' @return A heatmap of selected CT genes expression in single cells from fetal
#'     germ cells. If `values_only = TRUE`, a SingleCellExperiment is returned 
#'     instead.
#'
#' @export
#'
...
fetal_germcells_expression <- function(
    ...
    column_ha_type <- HeatmapAnnotation(
        type = df_col$type,
        border = TRUE,
        col = list(type = Fetal_cell_colors),
    ...
    h <- Heatmap(mat[, rownames(df_col), drop = FALSE],
        name = "logCounts",
        column_title = "Expression in Fetal germ cells (scRNAseq)",
        ...
        col = colorRamp2(seq(scale_lims[1], scale_lims[2], length = 11),
                         legend_colors),
        top_annotation = c(column_ha_sex, column_ha_type),
