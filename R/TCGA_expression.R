...
#'
#' @param tumor TCGA tumor code. can be one of "SKCM", "LUAD", "LUSC", "COAD", "ESCA",
#' "BRCA", "HNSC", or "all" (default).
#'
...
#'
#' @return A heatmap of selected CT genes in TCGA samples.
...
#'
#' @examples
#' TCGA_expression(TCGA_TPM, tumor = "LUAD", genes = c("MAGEA1", "MAGEA3", "MAGEA4"), units = "log_TPM")
TCGA_expression <- function(database, tumor = "all", genes = NULL, units = "TPM") {
  ...
  if (!tumor %in% c("SKCM", "LUAD", "LUSC", "COAD", "ESCA", "BRCA", "HNSC", "all")) {
  ...
  if(!is.null(genes)) {
    if(all(genes %in% rowData(data)$external_gene_name) == FALSE){
      ...
      cat(paste0(genes[!genes %in% rowData(data)$external_gene_name], " is not in the database.\n"))
  ...
  colData(data)[colData(data)$definition == "Solid Tissue Normal", "definition"] <- "Peritumoral"
  colData(data)[colData(data)$definition == "Primary solid Tumor", "definition"] <- "Primary"
  df_col <- data.frame("barcode" = colData(data)$barcode,
                       "shortLetterCode" = colData(data)$shortLetterCode,
                       "tumor" = sub("TCGA-", x = colData(data)$project_id, replacement = ''),
                       "type" = factor(colData(data)$definition,
                                       levels = c("Peritumoral", "Primary", "Metastatic")))
  ...
  column_ha_type <- HeatmapAnnotation(
    Type = df_col$type,
    border = TRUE,
    col = list(Type = c("Metastatic" = "firebrick2", "Primary" = "salmon", "Peritumoral" = "green")),
  ...
  column_ha_tumor <- HeatmapAnnotation(
    Tumor = df_col$tumor,
    border = TRUE,
    col = list(Tumor = c("BRCA" = "pink", "COAD" = "black", "ESCA" = "green", "HNSC" = "steelblue",
                         "LUAD" = "cyan", "LUSC" = "gray50", "SKCM" = "moccasin")),
    annotation_name_gp = gpar(fontsize = 8),
    annotation_legend_param = legends_param)

  split_order_by_cat <- factor(df_col$tissue, levels = c("Peritumoral", "Tumor"))
  if (tumor == "all"){
    split_order_by_cat <- factor(df_col$tumor,
                                 levels = c("BRCA", "COAD", "ESCA", "HNSC", "LUAD", "LUSC", "SKCM"))
  }
