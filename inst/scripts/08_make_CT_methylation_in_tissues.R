...
hg <- rtracklayer::import(
  "../../../CTdata/extdata/Homo_sapiens.GRCh38.dna.primary_assembly.fa",
  format = "fasta")
...
CT_list_prom_gr <- makeGRangesFromDataFrame(CT_promoter_regions,
                                            keep.extra.columns = TRUE,
...
CpG_positions_gr <- subsetByOverlaps(CpG_positions_gr, CT_list_prom_gr)



################################################################################
## Create a table with methylation values of CpG located in CT promoters
## in normal tissues
################################################################################
...
for (cell in Encode_tissues) {
  ...
  bismark_file <- paste0("../../../CTdata/extdata/", cell, ".bed")
...
bismark_file <- "../../../CTdata/extdata/SRR15427118_1_val_1_bismark_bt2_pe.bismark.cov.gz"
bismark <- read_tsv(bismark_file, col_types = cols(.default = "?", X1 = 'c'),
                    col_names = FALSE)
...
CT_list_met_normal_tissues_gr <-
  makeGRangesFromDataFrame(CT_met_in_tissues,
