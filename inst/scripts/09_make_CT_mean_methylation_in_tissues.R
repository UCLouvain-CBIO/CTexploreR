...
prom_mean_met_in_tissues <- tibble(tissue = c(colnames(CT_methylation_in_tissues), "CpG_number"))
for (gene in CT_list$external_gene_name) {
  ...
  promoter_methylation <- subsetByOverlaps(CT_methylation_in_tissues, promoter_gr)
  tmp <- enframe(colMeans(assay(promoter_methylation), na.rm = TRUE), name = "tissue", value = gene)
...
CT_CpG_number <- prom_mean_met_in_tissues %>%
  ...
  pivot_longer(names_to = "external_gene_name", values_to = "CpG_number", -tissue) %>%
  dplyr::select(-tissue) %>%
...
prom_mean_met_in_tissues <- prom_mean_met_in_tissues %>%
  filter(tissue != "CpG_number") %>%
  pivot_longer(names_to = "external_gene_name", values_to = "mean_methylation", -tissue) %>%
...
methylation_analysis <- tibble(external_gene_name = prom_mean_met_in_tissues$external_gene_name,
                               ...
                               sperm_met = prom_mean_met_in_tissues %>%
                                 dplyr::select(sperm) %>% pull(sperm))

methylation_analysis <- methylation_analysis %>%
  left_join(CT_list %>% dplyr::select(external_gene_name, ensembl_gene_id)) %>%
  ...
  mutate(CpG_promoter = case_when(CpG_density < 2 ~ "CpG_low",
                                 CpG_density >= 2 & CpG_density < 4 ~ "CpG_intermediate",
                                 CpG_density >= 4 ~ "CpG_high")) %>%
  mutate(methylation_in_tissues = case_when(somatic_met < 50 ~ "unmethylated_in_somatic",
                                          somatic_met >=50 & ratio_somatic_sperm > 2 ~ "methylated_in_somatic_unmethylated_in_germline",
                                          somatic_met >=50 & ratio_somatic_sperm <= 2 ~ "methylated_in_somatic_and_germline")) %>%
  dplyr::select(ensembl_gene_id, external_gene_name, CpG_density, CpG_promoter, methylation_in_tissues)
