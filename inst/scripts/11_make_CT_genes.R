...
#####################################################################################
## Based on DAC induction and on methylation levels in normal tissues (if available).
## For some genes, methylation analysis was not possible due to multimapping issues.
## In this case, genes are still considered as regulated by methylation if they
## show a strong activation in cells treated with 5-Aza-2′-Deoxycytidine.
#####################################################################################

CT_genes <- CT_genes %>%
  mutate(regulation = case_when(
    DAC == "induced" &
      (is.na(methylation_in_tissues) | methylation_in_tissues == "methylated_in_somatic_unmethylated_in_germline")
    ~ "methylation"))
...
#####################################################################################
CT_genes <- CT_genes %>%
  dplyr::select("ensembl_gene_id", "external_gene_name", "family", "X_linked", "GTEX_category",
                "TPM_testis", "max_TPM_somatic", "q75_TPM_somatic", "multimapping_analysis", "testis_specificity",
                "percent_of_positive_CCLE_cell_lines", "percent_of_negative_CCLE_cell_lines", "max_TPM_in_CCLE",
                "CCLE_category", "percent_pos_tum", "percent_neg_tum", "max_TPM_in_TCGA", "TCGA_category",
                "DAC", "methylation_in_tissues", "regulation", "met_exp_corr_TCGA", "CpG_density",
                "CpG_promoter", "external_transcript_name", "ensembl_transcript_id", "chromosome_name",
                "strand", "transcription_start_site", "transcript_length", "transcript_biotype", "oncogene", "tumor_suppressor")
