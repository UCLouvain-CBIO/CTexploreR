...
(GTEx, CCLE, TCGA and HPA) and summarises their main characteristics. Several
...
(Guo et al., 2018). 
`r table(CT_genes$external_gene_name%in%rownames(CTdata::testis_sce()))[[2]]`
genes out of our `r nrow(CTexploreR::CT_genes)` CT genes were in the dataset 
so we have not been able to determine the cell type for all genes. For each 
gene, the testis cell type has been determined as the cell type showing the 
highest mean expression for that gene, if they are detected (count > 0) in at 
least 1% of that specific cell-type. This allowed us to remove the genes for 
which this cell type corresponds to a testis somatic cell type (macrophage, 
